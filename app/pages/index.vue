<template>
  <main class="min-h-screen bg-gray-50">
    <div class="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
      <div class="mb-8 flex items-center justify-between gap-4">
        <div>
          <h1 class="text-3xl font-bold tracking-tight text-gray-900">
            {{ searchKeyword ? '搜索结果' : '博文列表' }}
          </h1>
          <p class="mt-2 text-sm text-gray-500">
            {{ searchKeyword ? `正在搜索：${searchKeyword}` : '浏览所有已发布的文章' }}
          </p>
        </div>
      ...
      >
        {{ searchKeyword ? '没有找到匹配的文章' : '还没有已发布的文章' }}
      </div>
...
const postListUrl = computed(() => {
  if (!searchKeyword.value) return '/api/posts/postlist'
  return `/api/posts/postlist?q=${encodeURIComponent(searchKeyword.value)}`
})
...
const {
  ...
  error,
  refresh
} = await useFetch<PostListItem[]>(postListUrl, {
  default: () => [],
  watch: [searchKeyword]
})
...
useSeoMeta({
  title: () => searchKeyword.value ? `搜索：${searchKeyword.value}` : '首页',
  description: () => searchKeyword.value ? `搜索包含 ${searchKeyword.value} 的博客文章` : '查看所有博客文章'
})
</script>
